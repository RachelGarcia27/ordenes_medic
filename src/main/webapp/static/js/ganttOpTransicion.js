...
var records = [];
var origenSegmento;
var fechaInicial, fecha_x, fechaExtendida,fechaSugerida;
var selectInicio = null;
var selectFinal = null;
var hours = null;
var idEscSelect = null;
...
function drawLabels(svg) {
    ...
    var nameGantt = name === 'Operadores' ? 'Op -' : 'Un -';
      var gOperador = svg.group({'class': ('pantalla_activo')});
    var gOperadorSig = svg.group({'class': ('pantalla_siguiente')});
    svg.rect(gOperador, originX,labelY+22,45, (labelY + rowHeight * (numRec + 1))-50);
    svg.rect(gOperadorSig, originX+45,labelY+22,50, (labelY + rowHeight * (numRec + 1))-50);
    for (i = 0; i <= numRec + 1; i++) {
        ...
        if (1 < i && i <= numRec + 1) {
            var operadorActEsc = $.grep(listaServicios,function(e) { return e.idUnidadGantt === records[i-2] && e.idEsc !== idEscSelect;});
            if(operadorActEsc.length>0){
            svg.text(gLbl, originX + 5,
                    (labelY - 5) + (rowHeight * (i)),
                    operadorActEsc[0].kRol+'-'+ operadorActEsc[0].idOperador);
                }         
        } else if (i === 1) {
            svg.text(gLbl, originX + 25,(labelY - 5) + (rowHeight * (i)),
                    name);
        }
    }
    //operador siguiente
    for (i = 0; i <= numRec + 1; i++) {
        ...
        if (1 < i && i <= numRec + 1) {
            var operadorActEsc = $.grep(listaServicios,function(e) { return e.idUnidadGantt === records[i-2] && e.idEsc === idEscSelect;});
            if(operadorActEsc.length>0){
            svg.text(gLbl, originX + 50,(labelY - 5) + (rowHeight * (i)),operadorActEsc[0].kRol+'-'+ operadorActEsc[0].idOperador);
        }
        } 
    }
}
...
function dibujaServicio(srvData, clicked) {
    ...
    var id = origenSegmento === 1 ? 'idUnidad' : 'idUnidadGantt';
    ...
    srvSVG.title(gBus, 'idgantt: ' + srvData.idUnidadGantt+ ', sec: '+ srvData.numeroSecuenciaDesarrollo+', idOp: '+ srvData.idOperador); 
        //evento   click
...
function drawGrid(svg) {
    ...
    var j, x, y;
    //color de fondo escenario Activo y  Siguiente
    var gEscActivo = srvSVG.group(gServicios, {
        'class': ('pantalla_activo')});
     var gEscSiguiente = srvSVG.group(gServicios, {
        'class': ('pantalla_siguiente')});
    srvSVG.rect(gEscActivo, originX,52,paramGANTT.colWidth * 72, boxHeight-60);
    srvSVG.rect(gEscSiguiente, paramGANTT.colWidth * 72+originX,52,paramGANTT.colWidth * 72, boxHeight-60);
    // Lineas Verticales
    for (i = 0; i <= hours; i++) {
        x = originX + paramGANTT.colWidth * i;
        clase = (i === 0 || i === hours ? "a5-vertical" : (i%24 === 0 ? "a5-midnight" : "a5-hour-line"));
        svg.line(g, x, originY-25, x, originY + rowHeight * (numRec + 1), {class: clase});
        ...
        if((i !== 192) && (i%24 === 0) && paramGANTT.type === 'semanal' ){
            ...
            if(i % 72 === 0 &&  i !== 72){
                ...
                svg.text(gTxt, x + 174, originY - 10, 'ESC ACTIVO');
                srvSVG.rect(gEscActivo, x + 144,originY-23,paramGANTT.colWidth * 24, 23);
            }
            if(i % 72 === 0 &&  i === 72){
                svg.text(gTxt, x + 174, originY - 10, 'ESC SIGUIENTE');
                srvSVG.rect(gEscSiguiente, x + 144,originY-23,paramGANTT.colWidth * 24, 23);
            }
        }
    }
    // linea  horizaontal
    
     if(paramGANTT.type === 'dia' || paramGANTT.type === 'semanal'){
    svg.line(g, originX, 6, gridWidth - 10, 6, {class: 'a5-horizontal'});
     }
    // Lineas Horizontales
    for (i = 0; i <= numRec + 1; i++) {
        ...
        // Viajes
        if (i < numRec) {
            var id = origenSegmento === 1 ? 'idUnidadGantt' : 'idOperador';
